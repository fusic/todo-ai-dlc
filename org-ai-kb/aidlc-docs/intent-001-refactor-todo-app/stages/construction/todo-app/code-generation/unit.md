...
> copy-forward: infrastructure-design 版 `unit.md` を全節不変のままコピーし、末尾に
> 「実装ステータス（code-generation で追記）」のみを追記した（blueprint identity 維持）。
> 原本: `stages/construction/todo-app/infrastructure-design/unit.md`。
>
> ＜infrastructure-design 時点のヘッダ（保存）＞
> Stage: infrastructure-design / Owner: aidlc-systems-architect-agent / 2026-06-10
> copy-forward: nfr-design 版 `unit.md` を全節不変のままコピーし、末尾に「デプロイトポロジー / IaC モジュール
> 参照 / ランタイム構成 / 運用所有」と「OQ 解決状況（残余ゼロ確認）」のみを追記した（blueprint identity 維持）。
> 原本: `stages/construction/todo-app/nfr-design/unit.md`。
>
> ＜nfr-design 時点のヘッダ（保存）＞
...
## デプロイトポロジー（infrastructure-design で追記 — 正は infrastructure-specification.md）

- **単一スタック `TodoStack`**（ap-northeast-1 / 単一環境 — C-4 / A-5）に全リソースが帰属。既存リソース（S3 / CloudFront / API GW / Lambda / DynamoDB / アクセスログ）は据置、本ステージの新規は **OriginVerifySecret（Secrets Manager）/ AlarmTopic（SNS）/ CloudWatch アラーム 4 種 / Lambda 明示 LogGroup** の 4 区分のみ
- **意図経路の単一化（QT-4）**: ブラウザ → CloudFront（default = S3 OAC、`/api/*` = API GW オリジン + カスタムヘッダ `x-origin-verify`）。execute-api 直接アクセスは検証ミドルウェアにより全操作 403。`ApiUrl` CfnOutput は検証手段として維持・説明文更新（Q1=a）
- **VPC なし** — 全マネージドサービス（QT-3。infrastructure テスト IT-10 でガード）
- デプロイ単位は unit 全体（CMP-001 dist + CMP-002 バンドル + IaC を 1 回の `cdk deploy` で同時配備 — Packaging assumption どおり）

## IaC モジュール参照（infrastructure-design で追記）

| モジュール | 内容 | 変更 |
|---|---|---|
| `packages/infrastructure/bin/app.ts` | CDK アプリエントリ（`npx tsx` 実行） | 変更なし |
| `packages/infrastructure/lib/todo-stack.ts` | 全リソース定義（単一スタック） | 変更明細 CH-1〜CH-10（infrastructure-specification.md）— deprecated 移行 / IAM 5 アクション / corsPreflight 削除 / Secret + ヘッダ注入 / アラーム + SNS / CfnOutput 整理 |
| `packages/infrastructure/test/`（新設） | infrastructure テスト（Template assertions IT-1〜IT-10） | 実装 = code-generation |
| CDK バージョン | 現行版・完全固定宣言維持（C-2 / RF-18 / SECURITY-10） | 更新実施 = code-generation |

## ランタイム構成（infrastructure-design で追記）

| 項目 | 値 | 備考 |
|---|---|---|
| Lambda | Node.js 20 / 256MB / 30s / esbuild ESM + source map | 据置（C-1） |
| Lambda 環境変数 | `TABLE_NAME` / `NODE_OPTIONS`（据置）+ **`ORIGIN_VERIFY_SECRET`**（Secrets Manager 動的参照 — 新規） | dev 値 `local-dev-only` はローカル構成 3 箇所（compose / Vite proxy / Playwright）が注入（C-5。実装 = code-generation） |
| オリジン検証ヘッダ | **`x-origin-verify`**（本ステージで名称確定） | CloudFront `/api/*` オリジンが付与・CMP-002 ミドルウェアが検証 |
| DynamoDB | `TodoTable` 固定名 / オンデマンド / PITR（`pointInTimeRecoverySpecification` へ移行） | 据置 + RF-17 移行 |
| ログ | Lambda = 明示 LogGroup（CDK 一意名・90 日・DESTROY）。旧 `/aws/lambda/<関数名>` は残置許容（Q2=a — 90 日で自然失効）。API アクセスログ据置 | RF-17 |
| アラーム | 4 種（Duration p95 > 500ms 3/3 / Latency p99 > 1500ms 3/3 / Errors ≥ 1 1/1 / 5xx ≥ 1 1/1。period 5 分・missing = notBreaching）→ SNS AlarmTopic | D-4 / D-5 |

## 運用所有（infrastructure-design で追記）

- **手動ステップは 2 つのみ維持**: ① SNS 購読（`AlarmTopicArn` 出力 + README 1 行 — D-5）② GitHub ブランチ保護（RF-01）。旧ロググループの手動削除は**追加しない**（Q2=a）
- デプロイ所有: 1 コマンド手動デプロイ（RF-20 — `frontend build → cdk deploy` の順序制約はスクリプトが内包）。CI は検証のみで AWS デプロイ権限を持たない
- ロールバック: CloudFormation 自動ロールバック + 直前コミット再デプロイ。データは PITR
- コスト増分: Secrets Manager 約 +$0.40/月のみ有意（nfr-design 見積りから変更なし）

## OQ 解決状況 — 残余ゼロ確認（infrastructure-design で追記）

| OQ / 申し送り | 状態 |
|---|---|
| OQ-1（RF-16 ローカル経路 + Secret 可視性） | nfr-design D-1/D-2 で解決済み → 本ステージで construct 設計まで確定（Secret / customHeaders / env / ヘッダ名）。残余 = 実装のみ（code-generation） |
| OQ-2（audit 閾値） | nfr-design D-3 で解決済み。本ステージ管轄分なし |
| OQ-3（アラーム統計・評価期間） | nfr-design D-4/D-5 で解決済み → 本ステージでリソース定義に翻訳完了。残余 = 実装のみ（code-generation） |
| 本ステージ新規裁量 | Q1（ApiUrl 維持 + 説明文更新）/ Q2（旧ロググループ残置）— 回答済み・成果物へ反映済み。**未解決の設計上の OQ はゼロ** |

## 実装ステータス（code-generation で追記 — 2026-06-10。ID → ファイルの全対応は implementation-map.md が正）

### RF 別実装状況（RF-01〜22 全件完了）

| フェーズ | RF | 状態 | 主要ファイル |
|---|---|---|---|
| P0 | RF-01（CI 6 ジョブ）/ RF-02（E2E）/ RF-03（shared） | 完了（コミット c8c95c8） | `.github/workflows/ci.yml` / `e2e/todo-smoke.spec.ts` + `playwright.config.ts` / `packages/shared/` |
| P1 | RF-04〜13（backend / frontend 実装品質） | 完了（コミット 185e7b4） | `packages/backend/src/`（readJsonBody / 条件付き書込 / sortForList / Powertools ログ / CORS 撤去）・`packages/frontend/src/`（actionError / createdAt 表示 / fetchTodo 削除 / build 分離） |
| P2 | RF-14/15/17（IaC）+ RF-16（オリジン検証） | 完了 | `packages/infrastructure/lib/todo-stack.ts`（CH-1〜CH-10）+ `test/todo-stack.test.ts`（IT-1〜IT-10）+ `packages/backend/src/middleware/originVerify.ts` + ローカル注入 3 箇所 |
| P2 | RF-18（依存メジャー 4 系統） | 完了 | Vitest ^3.2.6（4 パッケージ）/ Vite ^7.3.5 + @vitejs/plugin-react ^5.2.0 / Biome 2.4.16（migrate 済み）/ aws-cdk-lib 2.258.1 + aws-cdk 2.1126.0（完全固定維持 — C-2）。audit ignore 0 件（GHSA-5xrq-8626-4rwp 解消・削除）。C-7: node:20-slim = v20.20.2 ≥ 20.19 確認 |
| P2 | RF-19（Renovate）/ RF-20（deploy 1 コマンド）/ RF-21（ローカル堅牢化）/ RF-22（文書一致） | 完了 | `renovate.json` / root `deploy` script + README デプロイ節（SNS 購読 1 行）/ `Dockerfile.dev`（非 root・lockfile 強制）+ `dev.ts`（PORT）/ v1 `aidlc-docs/` 3 点修正 + バナー 5 ファイル |

### 検証証跡（最終 — write-test-verify）

- `pnpm lint`（Biome 2.4.16）green / `pnpm typecheck` 4 パッケージ green / `pnpm test` **99 件 green**（shared 16 / backend 42 / frontend 24 / infrastructure 17）/ `pnpm build` green
- `cdk synth` exit 0（deprecation 警告 0・`Custom::LogRetention` 0 件・Secrets Manager 動的参照 2 箇所・平文 0 件）
- `pnpm audit --audit-level=high` exit 0（ignore 0 件）
- E2E（クリーン volume + 新 Dockerfile.dev で再ビルドした docker compose 環境）**2/2 green**。実機確認: ヘッダなし直接アクセス 403 / dev ヘッダ付き 200 / 誤ヘッダ 403
- BP-1: BT-1〜7 不変（unit + E2E）。振る舞い変化は承認済み許容変更 1〜5 の範囲のみ（P2 の新規変化は許容変更 3 = 直接アクセス 403 のみ）

### 残余（orchestrator / 人間へ）

1. git commit / push（P2 全変更 — orchestrator 管轄）と push 後の CI 実体（GitHub Actions 6 ジョブ）green 確認
2. ブランチ保護の有効化（手動 — README 記載）と AWS 実デプロイ（`pnpm run deploy`）+ SNS 購読 1 行（手動 2 ステップ）
3. Node 20 系の保守終了接近（AWS SDK が node >=22 を推奨する警告を出力）— 実行基盤メジャー更新は BP-1 範囲外のため将来 intent
